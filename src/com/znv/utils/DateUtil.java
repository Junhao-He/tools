...
public class DateUtil {
	...
	public static Date getNowDate() {
		...
		Date currentTime_2 = format.parse(dateString, pos);
	...
	public static Date getNowDateShort() {
		Date currentTime = new Date();
		String dateString = format_date.format(currentTime);
		ParsePosition pos = new ParsePosition(8);
		Date currentTime_2 = format_date.parse(dateString, pos);
		return currentTime_2;
	}
	...
	public static String getTimeShort() {
		Date currentTime = new Date();
		String dateString = format_time.format(currentTime);
		return dateString;
	}
	...
	public static String getTwoDay(String sj1, String sj2) {
		long day = 0;
		try {
			java.util.Date date = format_date.parse(sj1);
			java.util.Date mydate = format_date.parse(sj2);
	 ...
	 * @return
	 */
	public static String toDate(byte[] bytes) {
		if (bytes == null || bytes.length != 6) {
			return "";
		}
		int[] ints = new int[bytes.length];
		for (int i = 0; i < bytes.length; i++) {
			ints[i] = ByteUtil.byte2Int(bytes[i]);
		}
		//yyyy-MM-dd HH:mm:ss
		return String.format("20%02d-%02d-%02d %02d:%02d:%02d", ints[0], ints[1], ints[2], ints[3], ints[4], ints[5]);
	}
	 ...
	 * @return
	 */
    public static String toStringDate(byte[] bytes) {
        if (bytes == null || bytes.length != 6) {
            return "";
        }
        //yyyy-MM-dd HH:mm:ss
        return String.format("20%02x-%02x-%02x %02x:%02x:%02x", ByteUtil.byte2Int(bytes[0]),ByteUtil.byte2Int(bytes[1]),
                ByteUtil.byte2Int(bytes[2]), ByteUtil.byte2Int(bytes[3]), ByteUtil.byte2Int(bytes[4]), ByteUtil.byte2Int(bytes[5]));
    }
