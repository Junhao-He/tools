...
public class GetDataFeature {
	...
	public static JSONObject getImageBatchFeature2(HashMap<String, String> nameMap, String url) {
		String result = null;
		CloseableHttpClient client = HttpClients.createDefault();
		// CloseableHttpClient client = HttpClientPool.getHttpClient(); // TODO
		// 检测连接池
		HttpPost httpPost = new HttpPost(url);
		MultipartEntityBuilder entity = MultipartEntityBuilder.create();
		...
		for (String filePath : nameMap.keySet()) {
			...
			// byte[] filedata = PictureUtils.image2byte(filePath);
			// entity.addBinaryBody("imageDatas", filedata,
			// ContentType.DEFAULT_BINARY, uuidName);
	...
	public static List<String> getAllPic(File fileDir) {
		...
		return listALlFile(fileDir, picFilepath);
	}
...
//		Logger logger = LogManager.getLogger(GetDataFeature.class);
		...
		// logger.warn(getImageFeature(filePath, url));
		 String feature = getImageFeature(filePath, url);
		 System.out.println(feature);
		 String feature1 = JSON.parseObject(feature).getString("feature");

		List<String> pics = getAllPic(new File(filePath));
		HashMap<String, String> map = new HashMap<>();

		for (int i = 0; i <pics.size(); i++) {
			String uuid = UUID.randomUUID().toString().replace("-", "");
			map.put(pics.get(i), uuid);
		}

//		JSONObject feature1 = getImageBatchFeature2(map, url);
